...
python3 run.py \
--app gen \
--model_path models/lsun_bedroom_200k_png.pkl \
--out_image_dir gen/lsun_bedroom_200k_png/ \
--num_pngs 5000 \
--gen_seed 0
"""
#----------------------------------------------------------------------------
...
class TrainingSchedule:
    def __init__(
        ...
        # 一定要注意这里！将minibatch的size改为了1，是为了更好的计算dct
        self.minibatch = 1
...
def train_progressive_gan(
    ...
    #print("#sched.minibatch: ",sched.minibatch)
    # Construct networks.
    with tf.device('/gpu:0'):
        if resume:
            try:
                ...
                D = tfutil.Network('D', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, minibatch_size_np=sched.minibatch, **config.D)
                Gs = G.clone('Gs')
        else:
            print('Constructing networks...')
            resume_kimg = 0.0
    ...
    with tf.name_scope('Inputs'):
        ...
        #print("#labels.shape: ", labels.shape) labels.shape:  (?, 1)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            # tf.control_dependencies操作需要用到的, 在这里指定了需要先运行lod_assign_ops里面的操作，所以在读取checkpoints的时候会报错
            ...
            with tf.name_scope('D_loss'):#, tf.control_dependencies(lod_assign_ops):
                #D_loss, Fin_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu, **config.D_loss)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        for repeat in range(minibatch_repeats):
            for _ in range(D_repeats):
                #_, _, d_loss, f_loss = tfutil.run([D_train_op, Gs_update_op, D_loss, Fin_loss], {lod_in: sched.lod, lrate_in: sched.D_lrate, minibatch_in: sched.minibatch})
                #print("Total loss is: ",d_loss, "Fingerprints loss is ", f_loss)
