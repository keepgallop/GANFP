...
def D_paper(
    ...
    structure           = None,         # 'linear' = human-readable, 'recursive' = efficient, None = select automatically
    is_template_graph   = False,        # True = template graph constructed by the Network class, False = actual evaluation.
    minibatch_size_np   = 32,           # ! Self add--for dct extractor
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    def feature_extractor(image_batch, batch_size, extractor_dir):
        def dct2d(img_batch, bat_size):
            with tf.variable_scope("DCT_Extractor", reuse=tf.AUTO_REUSE):
                imgs_dct = []
                for img_idx in range(bat_size):
                    ...
                    imgs_dct.append(layers_dct)
                imgs_dct = tf.convert_to_tensor(imgs_dct)
            return imgs_dct
        def fingerprint2d(img_batch):
            ...
            return fin_feature
        images_dct = dct2d(img_batch=image_batch, bat_size=batch_size)
        images_fin = fingerprint2d(img_batch=images_dct)
        return images_fin
    feat_out = feature_extractor(image_batch=images_in, batch_size=minibatch_size_np, extractor_dir=extractor_dir)
    #----------------------------------------------------------------------------
    ...
    features_out = tf.identity(feat_out, name="features_out")
