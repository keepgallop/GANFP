...
def G_wgan_acgan(G, D, opt, training_set, minibatch_size,
    ...
    fake_scores_out, fake_labels_out, fake_features_out = fp32(D.get_output_for(fake_images_out, is_training=True))
...
def D_wgangp_acgan(G, D, opt, training_set, minibatch_size, reals, labels,
    ...
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    real_scores_out, real_labels_out, real_features_out = fp32(D.get_output_for(reals, is_training=True))
    fake_scores_out, fake_labels_out, fake_features_out = fp32(D.get_output_for(fake_images_out, is_training=True))
    ...
    with tf.name_scope('FingerprintPenalty'):
        ...
        fingerprints_penalty = tf.reduce_mean(tf.abs(real_features_out - fake_features_out),  axis=1, keep_dims=False)
        fingerprints_penalty = tfutil.autosummary('Loss/fingerprints_scores', fingerprints_penalty)
    ...
    with tf.name_scope('GradientPenalty'):
        ...
        mixed_scores_out, mixed_labels_out, mixed_features_out = fp32(D.get_output_for(mixed_images_out, is_training=True))
