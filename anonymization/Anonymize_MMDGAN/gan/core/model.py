...
class MMD_GAN(object):
    ...
    def set_loss(self, G, images):
        ...
        #print("#self.images.shape: ", self.images.shape) #real_data.shape:  (32, 128, 128, 3)
        #print("#self.G.shape: ", self.G.shape) #fake_data.shape:  (32, 128, 128, 3)
        fingerprint_weight = 10.0
        real_features_out = self.feature_extractor(self.images)
        fake_features_out = self.feature_extractor(self.G)
        #print("#real_features_out.shape: ", real_features_out.shape) #real_features_out.shape:  (32, 8192)
        #print("#fake_features_out.shape: ", fake_features_out.shape) #fake_features_out.shape:  (32, 8192)

        with tf.variable_scope('loss'):
            self.g_loss = mmd.mmd2(kerGI)
            self.d_loss = -self.g_loss
            
            #print("#self.d_loss.shape: ", self.d_loss.shape) #self.d_loss.shape:  ()
            fingerprints_penalty = tf.reduce_mean(tf.abs(real_features_out - fake_features_out))
            #print("#fingerprints_penalty.shape: ", fingerprints_penalty.shape) #fingerprints_penalty.shape:  ()
            self.d_loss += -fingerprints_penalty * fingerprint_weight
...
#        self.sess.run(self.lr.assign(self.config.learning_rate))
    ...
    def feature_extractor(self, image_batch):
        ...
        def fingerprint2d(img_batch):
            ...
            with tf.variable_scope("Fingerprint_extractor", reuse=tf.AUTO_REUSE) as scope:
            ...
            return fin_feature
        images_dct = dct2d(img_batch=image_batch)
        images_fin = fingerprint2d(img_batch=images_dct)
        return images_fin
    #----------------------------------------------------------------------------